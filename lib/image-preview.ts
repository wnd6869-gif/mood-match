import {
  ...
  PROFILE_PHOTO_FILE_NAME_BY_MIME_TYPE,
...
export async function processProfilePhoto(
  ...
  let blob: Blob;
  let contentType: ProfilePhotoMimeType;
  ...
  if (blob.size > PROFILE_PHOTO_MAX_SIZE_BYTES) {
    throw new ImagePreviewError(
  ...
  return {
    blob,
    contentType,
    fileName: PROFILE_PHOTO_FILE_NAME_BY_MIME_TYPE[contentType],
  };
}
